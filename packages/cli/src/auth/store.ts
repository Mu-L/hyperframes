...
/**
 * Tight legacy-plaintext heuristic. HeyGen API keys start with `hg_`
 * (e.g. `hg_...`) and only contain URL-safe characters. We deliberately
 * accept ONLY those — previously we accepted any printable ASCII 8+
 * chars, which silently lifted JSON fragments that had lost their
 * leading `{`, other products' API keys, or pasted Stripe / GitHub
 * tokens into the credential slot.
 */
function looksLikeApiKey(s: string): boolean {
  return /^hg_[A-Za-z0-9_-]{5,}$/.test(s);
}
