/**
 * OAuth 2.0 + PKCE driver for the HeyGen backend.
 *
 * Entry points:
 *   - `startAuthorizationCodeFlow()` — full interactive login (browser
 *     opens, loopback waits, code → tokens, persist).
 *   - `refreshTokens()` — POST /v1/oauth/token with grant_type=refresh_token.
 *   - `revokeTokens()` — best-effort POST /v1/oauth/revoke.
 *
 * Endpoints (backend-verified):
 *   - GET  /v1/oauth/authorize
 *   - POST /v1/oauth/token
 *   - POST /v1/oauth/revoke
 *
 * Public client — no `client_secret`. The `client_id` is baked into
 * the binary and can be overridden via `HYPERFRAMES_OAUTH_CLIENT_ID`
 * for dev/test.
 */
...
export interface AuthorizeFlowOptions {
  /** Override scopes (default `openid profile`). */
  scope?: string;
...
export async function refreshTokens(
  ...
  const res = await fetchImpl(`${apiBaseUrl()}/v1/oauth/token`, {
    method: "POST",
    headers: {
...
export async function revokeTokens(token: string, opts: RevokeOptions = {}): Promise<void> {
  ...
  try {
    const res = await fetchImpl(`${apiBaseUrl()}/v1/oauth/revoke`, {
      method: "POST",
...
function buildAuthorizeUrl(args: {
  ...
  return `${apiBaseUrl()}/v1/oauth/authorize?${params.toString()}`;
}
